...
import { Row, Col, Stack, Form, Button, Card, Badge, Modal } from "react-bootstrap"
...
const NoteList = ({availableTags, notes, onDeleteTag, onUpdateTag}: NoteListProps) => {
  ...
  return (
    ...
    <Row>
      ...
      <EditListModal onDeleteTag={onDeleteTag} onUpdateTag={onUpdateTag} show={editTagsModalIsOpen} handleClose={()=>setEditTagsModalIsOpen(false)} availableTags={availableTags} />
    </>
  )
}
     ...
     text-decoration-none ${styles.card}`}>
       <Card.Body>
        <Stack gap={2} className="align-items-center justify-content-center h-100">
            ...
            {tags.length > 0 && (<Stack gap={1} className="justify-content-center flex-wrap" direction="horizontal" >
                {tags.map(tag=>(
...
function EditListModal({availableTags, show, handleClose, onDeleteTag,onUpdateTag}:EditTagsModalProps){
    return(
